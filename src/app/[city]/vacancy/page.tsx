...
export async function generateMetadata({
  ...
  return {
    title: `${name}の保育園 空き状況【${asOf}時点】｜区・年齢でさがす【hoikaten】`,
    description: `${name}が公開する保育所等の入所状況（${asOf}時点）をもとに、市内${num(total.facilityCount)}施設の受入可能数・入所待ち人数を区と年齢で絞り込んで探せます。空きがある施設だけの表示にも対応。`,
    alternates: {
...
export default async function VacancyPage({
  ...
  const byWard = summarizeByWard(vacancy);
  const total = totalSummary(vacancy);
  const siteCount = vacancy.facilities.filter((f) => f.site).length;

  // 空きが申込を上回っている年齢（3歳以上で起きる）
  const surplusAges = byAge.filter((a) => a.vacancy > a.waiting);
  ...
  const faqItems: FaqItem[] = [
    ...
    },
    {
      ...
      answer:
        ...
        `${asOfLabel}時点では、市全体で空き枠${num(total.vacancy)}に対して申込が${num(total.waiting)}件あり、` +
        (tightestAge
    ...
    },
    {
      ...
      answer:
        `${name}が公開している「保育所等の入所状況」の${asOfLabel}時点のデータです。${name}は毎月1日時点の状況を公開しています（毎年3月1日時点の公開はありません）。` +
        `当サイトは公開を検知して自動で更新しています。申込状況により空き枠は日々変動するため、最新の状況は${name}の公式ページでご確認ください。`,
    },
    {
      question: `${name}で0歳児・1歳児の空きが少ないのはなぜですか？`,
      answer:
        `${asOfLabel}時点のデータでは、${AGE_LABELS[0]}は空き枠${num(byAge[0].vacancy)}に対して申込${num(byAge[0].waiting)}件、` +
        `${AGE_LABELS[1]}は空き枠${num(byAge[1].vacancy)}に対して申込${num(byAge[1].waiting)}件と、低年齢ほど申込が空き枠を大きく上回っています。` +
    ...
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      <div className="hero-pattern rounded-2xl py-8 px-4 text-center mb-6 -mx-4 sm:mx-0">
        ...
        <p className="text-muted-foreground">
          市内{num(total.facilityCount)}施設の空き枠を、区と年齢でさがせます
        </p>
      ...
      <section className="mb-8">
        ...
        <div className="mt-3 rounded-xl bg-muted/50 p-3.5">
          <p className="text-xs text-muted-foreground leading-relaxed">
            入所待ち人数は{name}の定義で「園ごとの申請数」です。1人が複数園を希望すると希望した各園に計上されるため、
            <span className="font-medium text-foreground">
              実際に入園を待っている人数や競争倍率とは一致しません
            </span>
            。申込がどれだけ集中しているかの目安としてご覧ください。
          </p>
        </div>
      </section>
          ...
          <table className="w-full text-sm min-w-[520px] px-4 sm:px-0">
            <thead>
              <tr className="border-b border-border text-left">
                <th className="py-2.5 px-3 font-medium">区</th>
            ...
            <tbody>
              ...
              <tr className="border-b-2 border-border font-bold">
                <td className="py-2.5 px-3 whitespace-nowrap">市全体</td>
                <td className="py-2.5 px-3 text-right tabular-nums">
      ...
      <section className="rounded-xl border border-border/60 bg-muted/30 p-4">
        ...
        <ul className="text-xs text-muted-foreground space-y-1.5 leading-relaxed">
          ...
          <li>
            公開されている受入可能数・入所待ち人数・入所児童数のCSVを施設番号で結合し、そのままの数値を掲載しています。
            「空き1枠あたりの申込数」のみ当サイトで算出したものです（入所待ち人数 ÷ 受入可能数）。
          </li>
          <li>
            {name}の注記のとおり、掲載されている人数はシステムで機械的に抽出されているため、実際の人数と異なる場合があります。
          </li>
          <li>
