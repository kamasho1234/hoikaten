  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      <nav className="text-sm text-muted-foreground mb-6 flex items-center gap-2 flex-wrap">
        ...
        <span>/</span>
        <a href={`/${city}/articles`} className="hover:underline hover:text-primary">
          記事一覧
        </a>
      </nav>

      {/* ヒーローセクション */}
      <div className="relative bg-gradient-to-br from-primary/5 via-primary/10 to-transparent rounded-2xl p-8 mb-8 overflow-hidden">
        <div className="absolute top-4 right-4 text-7xl opacity-10 select-none pointer-events-none">
          {article.emoji}
        </div>
        <div className="relative">
          <span className="inline-block bg-primary/15 text-primary text-xs font-medium px-3 py-1 rounded-full mb-3">
            {article.category}
          </span>
      ...
      <article>
        <div
          className="
            ...
            prose-h2:text-lg prose-h2:mt-12 prose-h2:mb-5 prose-h2:pb-2 prose-h2:border-b prose-h2:border-border prose-h2:flex prose-h2:items-center prose-h2:gap-2
      ...
      <div className="mt-10 bg-gradient-to-r from-primary/10 to-primary/5 rounded-2xl p-8 text-center">
        <p className="text-3xl mb-3">{article.emoji}</p>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {otherArticles.map((a) => (
              <a key={a.slug} href={`/${city}/articles/${a.slug}`}>
                <Card className="hover:border-primary/50 hover:shadow-sm hover:-translate-y-0.5 transition-all cursor-pointer h-full">
                  <CardHeader className="py-4">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-2xl">{a.emoji}</span>
                      <span className="bg-primary/15 text-primary text-[10px] font-medium px-2 py-0.5 rounded-full">
                        {a.category}
                      </span>
                    </div>
