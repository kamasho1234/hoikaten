...
export async function generateMetadata({
  ...
  if (!article) return {};
  return {
    title: `${article.title}｜保活コラム｜hoikaten`,
...
export default async function ArticlePage({
  ...
  const allArticles = getArticlesByCity("general");
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      <nav className="text-sm text-muted-foreground mb-6 flex items-center gap-2 flex-wrap">
        ...
        <span>/</span>
        <a href="/articles" className="hover:underline hover:text-primary">保活コラム</a>
      </nav>
      ...
      <div className="mt-8 space-y-3">
        ...
        <div className="flex justify-center gap-3">
          <a
            href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(article.title)}&url=${encodeURIComponent(`https://hoikaten.com/articles/${slug}`)}`}
            target="_blank"
          ...
          <a
            href={`https://social-plugins.line.me/lineit/share?url=${encodeURIComponent(`https://hoikaten.com/articles/${slug}`)}&text=${encodeURIComponent(article.title)}`}
            target="_blank"
            rel="noopener noreferrer"
      ...
      <div className="mt-8 bg-muted/30 rounded-xl p-4 border">
        <p className="text-xs text-muted-foreground leading-relaxed">
          ...
          {article.publishedAt}時点のものです。最新情報は各自治体の公式サイトをご確認ください。
