...
<p>豊橋市はフルタイム共働きの場合、父20点＋母20点＝基本指数40点が基準ラインです。補正指数の加点で差がつきやすいので、該当する項目がないか確認しましょう。</p>
  ...
  // 3. 就労証明書の書き方
  {
    ...
    citySlug: "toyohashi",
    title: "豊橋市の就労証明書｜様式ダウンロード先と記入例【令和8年度】",
    description:
      "豊橋市の保育園申込みに使う就労証明書について、公式の様式ダウンロード先へのリンクと、点数の付き方・記入例・よくある不備をまとめています。会社に依頼する前に確認してください。",
    category: "書類準備",
    categoryColor: "amber",
    image:
...
<p>就労証明書の様式・記入例は<a href="https://www.city.toyohashi.lg.jp/65202.htm" target="_blank" rel="noopener">豊橋市「入園の申込みに必要なもの」</a>からダウンロードできます。この記事では、その様式をどう書くと点数にどう響くかを説明します。</p>
</div>
<h2>就労証明書とは</h2>
<p>就労証明書は、保護者が仕事をしていることを証明する書類です。豊橋市の保育園入園申込みでは、父母それぞれの保育事由証明書が必要で、就労が理由の場合は雇用主に記入してもらいます。</p>

<h2>記入のポイント</h2>
<h3>就労時間・日数の記載</h3>
<p>豊橋市では、就労<strong>日数と1日の就労時間</strong>の組み合わせで点数が決まります。月20日以上か月16日以上か、そして1日7.5時間以上・6時間以上・4時間以上かで点数が異なります。</p>
...
<h2>提出先と提出方法</h2>
<p>必要書類は入園希望月ごとの申込期間中に第1希望園へ提出するか、保育課（豊橋市役所東館2階）で提出することもできます。</p>

<div class="info-box">
<p><strong>提出のタイミング</strong></p>
<p>雇用主への依頼は2〜3週間前に行いましょう。書類の不備がある場合、保育の必要性の指数判断ができず、就業形態の最低点から-1点で取り扱われることがあります。</p>
</div>`,
    publishedAt: "2026-04-05",
    popularity: 45,
  },
