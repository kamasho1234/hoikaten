...
// ---------------------------------------------------------------------------
// 盛岡市 保育園入園 基本点数・調整点数データ
// 出典：盛岡市保育施設利用調整基準 別表１ 点数表（令和8年度用）
// https://www.city.morioka.iwate.jp/kosodate/kodomo_azukeru/1053010.html
// https://www.city.morioka.iwate.jp/_res/projects/default_project/_page_/001/053/010/R8tensuuhyou.pdf
// ---------------------------------------------------------------------------
//
// 盛岡市の利用調整は以下のように算出されます。
//   合計点数 ＝ 父の基本点数 ＋ 母の基本点数 ＋ 調整点数（加算・減算）
// 父母それぞれの基本点数は、保育を必要とする事由のうち
// 最も該当するものを1つ選択して決定します。
// 基本点数は最大22点（入院の場合）、就労は最大20点です。
// ---------------------------------------------------------------------------
...
const employmentOptions = (prefix: string) => [
  ...
  { label: '月60時間以上80時間未満（14点）', value: `${prefix}_employment_14`, points: 14 },
  { label: '月48時間以上60時間未満（13点）', value: `${prefix}_employment_13`, points: 13 },
];
...
const familyWorkerOptions = (prefix: string) => [
  ...
  { label: '月60時間以上80時間未満（11点）', value: `${prefix}_familyworker_11`, points: 11 },
  { label: '月48時間以上60時間未満（10点）', value: `${prefix}_familyworker_10`, points: 10 },
];
...
const jobPlannedOptions = (prefix: string) => [
  ...
  { label: '就労予定：準母子または準父子家庭（11点）', value: `${prefix}_jobplanned_11`, points: 11 },
];
...
const illnessOptions = (prefix: string) => [
  ...
  { label: '入院（22点）', value: `${prefix}_illness_22`, points: 22 },
  { label: '常時臥床（20点）', value: `${prefix}_illness_20`, points: 20 },
  { label: '安静を要する状況（19点）', value: `${prefix}_illness_19`, points: 19 },
  { label: '精神にかかる疾病（精神障害者保健福祉手帳1級を除く）（18点）', value: `${prefix}_illness_18`, points: 18 },
  { label: '上記以外で通院が必要な状態（16点）', value: `${prefix}_illness_16`, points: 16 },
];
...
const disabilityOptions = (prefix: string) => [
  ...
  { label: '身体障害1・2級、精神障害者保健福祉手帳1級、療育手帳A、介護保険要介護4または5（20点）', value: `${prefix}_disability_20`, points: 20 },
];
...
const childbirthOptions = (prefix: string) => [
  ...
  { label: '産前8週（多胎の場合は14週）、産後8週（21点）', value: `${prefix}_childbirth_21`, points: 21 },
];
...
const careOptions = (prefix: string) => [
  ...
  { label: '在宅看護：常時観察・介護が必要な場合（20点）', value: `${prefix}_care_20b`, points: 20 },
...
const abuseOptions = (prefix: string) => [
  ...
  { label: '虐待やDVのおそれがあること（21点）', value: `${prefix}_abuse_21`, points: 21 },
];
...
function buildParentQuestions(parentNum: 1 | 2): Question[] {
  ...
  const reasonQuestion: Question = {
    ...
    helpText: '盛岡市では父母それぞれの基本点数を合算します。いちばん近いものをひとつ選んでください。',
    inputType: 'select',
    options: [
  ...
  const detailQuestions: Question[] = [
    ...
    },
    {
      ...
      helpText: '身体障害1・2級、精神障害者保健福祉手帳1級、療育手帳A、介護保険要介護4または5が対象です',
      inputType: 'radio',
...
const adjustmentQuestions: Question[] = [
  ...
  },
  {
    ...
    category: 'adjustment',
    label: '保護者が盛岡市内の認可保育施設に勤務していますか？',
    helpText: '保育教諭、幼稚園教諭、保育士、看護師等が対象です。新規勤務開始・育休復帰等の場合に限ります',
    inputType: 'radio',
    options: [
      { label: 'あてはまらない', value: 'adj_childcare_worker_no', points: 0 },
      { label: '盛岡市内の認可保育施設に勤務（+30）', value: 'adj_childcare_worker_30', points: 30 },
      { label: '盛岡市外の認可保育施設に勤務（+28）', value: 'adj_childcare_worker_28', points: 28 },
    ],
  },
  {
    id: 'adj_sotsuen',
    ...
    helpText: '小規模保育事業等の卒園児は+5点（事業所内保育の従業員枠は+3点、企業主導型保育所は+2点）',
    inputType: 'radio',
    options: [
      { label: 'あてはまらない', value: 'adj_sotsuen_no', points: 0 },
      { label: '卒園児（+5）', value: 'adj_sotsuen_5', points: 5 },
      { label: '事業所内保育事業所の従業員枠入所児童の卒園児（+3）', value: 'adj_sotsuen_3', points: 3 },
...
export const moriokaData: MunicipalityData = {
  municipality,
  questions: [
