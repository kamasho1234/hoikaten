...
// ---------------------------------------------------------------------------
// 江別市 保育園入園 基本指数・調整指数データ
// ---------------------------------------------------------------------------
// 江別市は「基本指数」＋「調整指数」の二段階方式。
// 父母それぞれに基本指数を算出し、合算（加算方式）で優先順位を判断。
// 各保護者の最大基本指数は20点（合計40点満点）。
// 出典: 令和8年度 江別市保育所等利用調整基準
// https://www.city.ebetsu.hokkaido.jp/site/kosodate/hoiku-riyou.html
// ---------------------------------------------------------------------------
...
const employmentOptions = (prefix: string) => [
  ...
  { label: '自営業（月160時間以上）', value: `${prefix}_employment_self_20`, points: 20 },
  { label: '自営業（月120時間以上160時間未満）', value: `${prefix}_employment_self_16`, points: 16 },
  { label: '自営業（月64時間以上120時間未満）', value: `${prefix}_employment_self_10`, points: 10 },
  { label: '内職（月64時間以上）', value: `${prefix}_employment_naishoku_8`, points: 8 },
  { label: '就労内定（入園月までに就労開始）', value: `${prefix}_employment_naitei_8`, points: 8 },
];
...
const adjustmentQuestions: Question[] = [
  ...
  },
  {
    ...
    category: 'adjustment',
    label: '保護者が江別市内の認可保育施設で保育士・保育教諭として就労（予定）していますか？',
    inputType: 'radio',
    options: [
