"""
都城市の「認可保育所・認定こども園等の受入れ状況一覧」PDFから表を抜き出す

実行: python scripts/miyakonojo-pdf-extract.py <pdf>
出力: 標準出力にJSON（fetch-miyakonojo-vacancy.ts から呼ぶ）
...
def extract(path):
    ...
    with pdfplumber.open(path) as pdf:
        ...
        m = re.search(r"令和(\d+)年(\d+)月分", flat)
        if not m:
            fail("「令和N年M月分」を読み取れませんでした")
