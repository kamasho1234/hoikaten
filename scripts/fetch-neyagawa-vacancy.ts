 ...
 * 寝屋川市の選考受入れ状況を取り込む
 *
 * 実行: npm run vacancy:fetch:neyagawa
 *
 * ## この自治体の特徴
 * - 記号は ◎＝6人以上、○＝3〜5人、△＝1〜2人、×＝受入枠なし
 * - エリア（西北・東北・東・西・西南・南）で絞り込める
 * - **月ごとのPDFが1ページに並ぶ**ので、「令和N年M月」がいちばん新しいものを選ぶ。
 *   4月だけは1次・2次・最終と複数あるので、同じ月なら最終選考を選ぶ
 * - PDFに「何日時点」は書かれていないので、公式ページの更新日を時点として使う
 */
...
const MUNICIPALITY_SLUG = "neyagawa";
const MUNICIPALITY_NAME = "寝屋川市";
const SOURCE_NAME = "寝屋川市「入園申請空き枠状況」";
const INDEX_URL =
  "https://www.city.neyagawa.osaka.jp/organization_list/kodomo/hoikuka/kodomo_hoikusyo/hoikusho/25327.html";
const AGE_COUNT = 6;
const MIN_FACILITIES = 40;
const UA = "Mozilla/5.0 (compatible; hoikaten/1.0; +https://hoikaten.com)";

const OUT_PATH = path.join(process.cwd(), "src", "lib", "vacancy", `${MUNICIPALITY_SLUG}.json`);
const EXTRACTOR = path.join(process.cwd(), "scripts", "neyagawa-pdf-extract.py");
...
type PdfResult = {
  ...
  blanks: number;
  rows: { area: string; name: string; marks: (string | null)[] }[];
};
...
async function main() {
  ...
  const links = [...html.matchAll(/<a[^>]+href="([^"]+\.pdf)"[^>]*>([\s\S]*?)<\/a>/gi)]
    ...
    .map((l) => {
      const m = /令和(\d+)年(\d+)月/.exec(l.text);
      // 4月は1次・2次・最終と並ぶので、最終選考を後ろに置く
      const stage = l.text.includes("最終選考") ? 2 : l.text.includes("2次選考") ? 1 : 0;
      return { ...l, key: m ? Number(m[1]) * 100 + Number(m[2]) : 0, stage };
    })
    .filter((l) => l.key > 0)
    .sort((a, b) => b.key - a.key || b.stage - a.stage);
  if (links.length === 0) fail("空き状況のPDFが見つかりません");
  const link = links[0];
  console.log(`PDF: ${link.text}\n  ${link.url}`);

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "neyagawa-vacancy-"));
  try {
    const r = await fetch(link.url, { headers: { "User-Agent": UA } });
    if (!r.ok) fail(`PDFの取得に失敗しました（${r.status}）: ${link.url}`);
    const buf = Buffer.from(await r.arrayBuffer());
    if (buf.subarray(0, 4).toString() !== "%PDF") fail(`PDFではありません: ${link.url}`);
    const file = path.join(tmpDir, "neyagawa.pdf");
    ...
    const notes = [
      `寝屋川市は空き状況を人数ではなく記号で公表しています。これは${targetLabel}選考の受入れ状況で、公式ページが${asOf}に更新されたものです。`,
      ...pdf.notes,
    ...
    console.log(`  ${facilities.length}施設 / ${wards.join("・")} / 空らん ${blanks}`);
  } finally {
