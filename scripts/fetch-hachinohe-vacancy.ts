 ...
 * 八戸市の保育施設の空き状況を取り込む
 *
 * 実行: npm run vacancy:fetch:hachinohe
 *
 * ## この自治体の特徴
 * - 記号（○＝空き枠がある、－＝空き枠がない）。空欄はそのクラスを設けていない
 * - 施設の類型は公表していないので categories は持たない
 * - 地区は縦結合だが罫線が引かれていない場所があるので、行のy座標で切って読む
 */
...
const MUNICIPALITY_SLUG = "hachinohe";
const MUNICIPALITY_NAME = "八戸市";
const SOURCE_NAME = "八戸市「保育施設空き状況一覧」";
const INDEX_URL =
  "https://www.city.hachinohe.aomori.jp/soshikikarasagasu/kodomomiraika/kosodate/1/hoikuriyou/2038.html";
const AGE_COUNT = 6;
...
const EXTRACTOR = path.join(process.cwd(), "scripts", "hachinohe-pdf-extract.py");
...
async function main() {
  ...
  const pdfs = [...html.matchAll(/<a[^>]+href="([^"]+\.pdf)"[^>]*>([\s\S]*?)<\/a>/gi)]
    ...
    .filter((l) => /保育施設空き状況一覧/.test(l.text));
  if (pdfs.length !== 1) fail(`空き状況のPDFが${pdfs.length}件見つかりました（1件のはず）`);
  const [pdfLink] = pdfs;
  console.log(`PDF: ${pdfLink.url}`);

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "hachinohe-vacancy-"));
  try {
    ...
    const file = path.join(tmpDir, "hachinohe.pdf");
    ...
    const [, targetMonth, targetDay] = pdf.target;
    console.log(`基準日: ${asOf} / 対象: ${targetMonth}月${targetDay}日入所審査後`);

    const symbolLegend = pdf.legend.map((l) => ({
      mark: shapeOf(l.mark),
      label: l.label,
      open: !/ない$/.test(l.label),
    }));
    ...
    const dataset = {
      ...
      metrics: ["symbol"],
      subtitle: `${targetMonth}月${targetDay}日入所の審査が終わった時点の空き状況`,
      notes: [
        "八戸市は空き状況を人数ではなく記号で公表しています。当サイトでも公式の記号のまま載せています。",
        "児童の入退所や各施設の事情などにより空き状況は変動するため、目安としてご利用ください。",
        "空きがあっても、保育の必要性の優先度が高い順に入所が決まるため、入所を確約できるものではありません。",
        "空きがない施設でも、退所などで空きができる場合があり、申込はできます。",
        "年齢はその年度の4月1日時点の満年齢です。設けていないクラスは「—」にしています。",
      ],
      wards,
      categories: [],
      symbolLegend,
