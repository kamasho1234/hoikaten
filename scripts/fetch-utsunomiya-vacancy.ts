...
type PdfRow = {
  ...
  town: string;
  marks: string[];
  /** 0歳児クラスが月齢で分かれている資料での「0-0歳」の記号。分かれていなければ null */
  young: string | null;
  closed: boolean;
};
...
async function main() {
  ...
  try {
    ...
    if (
      previous?.asOf === asOf &&
      previous?.sourceFiles?.vacancy === pdfUrl &&
    ...
    const notes = [
      ...
      "設けていないクラスは「—」にしています。",
      ...(pdf.rows.some((r) => r.young !== null && r.young !== undefined)
        ? [
            "宇都宮市は0歳児クラスを月齢で2つ（0-0歳・0-1歳）に分けて公表しています。" +
              "当サイトの「0歳」は他の自治体と同じく4月1日時点で0歳の子を指すため、" +
              "「0-1歳」（令和7年4月2日〜令和8年4月1日生まれ）の欄を載せています。" +
              "それより後に生まれたお子さんは、市の資料の「0-0歳」の欄をご確認ください。",
          ]
        : []),
    ];
