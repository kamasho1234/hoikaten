/**
 * 亀岡市の保育所等 入所受入可能状況を取り込む
 *
 * 実行: npm run vacancy:fetch:kameoka
 *
 * ## この自治体の特徴
 * - 記号は ○＝4人以上、△＝1〜3人、×＝空きなし、□＝保育士確保次第入所可、
 *   －＝保育課へ問い合わせ。**「□」のように人数以外の意味を持つ記号がある**
 * - **合同学級の施設は年齢の欄が結合されている**（東本梅の1・2歳と3〜5歳など）。
 *   結合された欄には同じ記号を広げて入れている
 * - PDFに「何日時点」がないので、公式ページの更新日を時点として使う
 * - 「※満1歳から受入可能」のような施設ごとの但し書きが空らんの欄に縦書きで
 *   重ねて印字されているが、文字の順序が崩れていて復元できないので取り込んでいない
 */
...
const EXTRACTOR = path.join(process.cwd(), "scripts", "kameoka-pdf-extract.py");
...
type PdfResult = {
  ...
  textMarkCounts: Record<string, number>;
  blanks: number;
  expanded: number;
  rows: { kind: string; name: string; marks: (string | null)[] }[];
};
...
async function main() {
  ...
  // PDFに「何日時点」がないので、ページの更新日を時点として使う
  const updated = /class="date">(\d{4})年(\d{1,2})月(\d{1,2})日更新/.exec(toHalfWidth(html));
  ...
  const links = [...html.matchAll(/<a[^>]+href="([^"]+\.pdf)"[^>]*>([\s\S]*?)<\/a>/gi)]
    ...
    .filter((l) => l.text.includes("受け入れ可能状況"));
  if (links.length !== 1) fail(`受け入れ可能状況のPDFが${links.length}件あります（1件のはず）`);
  const link = links[0];
  console.log(`PDF: ${link.text}\n  ${link.url}`);

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "kameoka-vacancy-"));
  try {
    const r = await fetch(link.url, { headers: { "User-Agent": UA } });
    if (!r.ok) fail(`PDFの取得に失敗しました（${r.status}）: ${link.url}`);
    const buf = Buffer.from(await r.arrayBuffer());
    if (buf.subarray(0, 4).toString() !== "%PDF") fail(`PDFではありません: ${link.url}`);
    const file = path.join(tmpDir, "kameoka.pdf");
    ...
    // 検算1: 表に印字されている記号の数＋凡例の記号の数が、ページ全体の記号の数と合うか
    const inTable = Object.values(pdf.markCounts).reduce((a, b) => a + b, 0);
    const inPage = Object.values(pdf.textMarkCounts).reduce((a, b) => a + b, 0);
    if (inTable + pdf.legend.length !== inPage) {
      fail(
        `記号の数が合いません（表${inTable}＋凡例${pdf.legend.length} / ページ全体${inPage}）`
      );
    }
    // 検算2: 取り込んだ記号の数が、表の記号＋合同学級で広げたぶんと合うか
    const total = [...marks.values()].reduce((a, b) => a + b, 0);
    if (total !== inTable + pdf.expanded) {
      fail(`記号の数が合いません（表${inTable}＋広げたぶん${pdf.expanded} / 取り込み ${total}）`);
    }
    ...
    console.log(
      `記号の数はPDFの印字と一致しました（合同学級で広げたぶん ${pdf.expanded}欄をふくむ）`
    );
    ...
    const notes = [
      `亀岡市は空き状況を人数ではなく記号で公表しています。これは${targetLabel}入所希望者向けのもので、公式ページが${asOf}に更新されたものです。`,
      ...pdf.notes,
      "合同学級の年齢には、公式の表の結合されたらんと同じ記号を入れています。",
      "公式の表で空らんになっている年齢は「—」にしています。その年齢のクラスがないことを表しています。公式の表には「満1歳から受入可能」のような但し書きが添えられている施設があるので、くわしくは公式のPDFをご確認ください。",
    ];
    ...
    const dataset = {
      ...
      metrics: ["symbol"],
      subtitle: `${targetLabel}入所希望者向けの受入可能状況`,
      notes,
      wards: [],
      categories,
      symbolLegend,
