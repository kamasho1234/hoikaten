 ...
 * 東広島市の保育施設空き状況を取り込む
 *
 * 実行: npm run vacancy:fetch:higashihiroshima
 *
 * ## この自治体の特徴
 * - 記号は ◎＝10人以上、〇＝5〜9人、△＝1〜4人、×＝空き無し。
 *   **人数の幅で示す**ので、当サイトでは人数ではなく記号として載せる
 * - PDFが**左右2段組み**（左が公立、右が私立）
 * - 空らんは、そのクラスがない施設のもの
...
const MUNICIPALITY_SLUG = "higashihiroshima";
const MUNICIPALITY_NAME = "東広島市";
const SOURCE_NAME = "東広島市「市内保育施設の入所状況について」";
const INDEX_URL = "https://www.city.higashihiroshima.lg.jp/soshiki/kodomomirai/2/7/18970.html";
const AGE_COUNT = 6;
const MIN_FACILITIES = 50;
const UA = "Mozilla/5.0 (compatible; hoikaten/1.0; +https://hoikaten.com)";

const OUT_PATH = path.join(process.cwd(), "src", "lib", "vacancy", `${MUNICIPALITY_SLUG}.json`);
const EXTRACTOR = path.join(process.cwd(), "scripts", "higashihiroshima-pdf-extract.py");
...
type PdfResult = {
  ...
  blanks: number;
  rows: { code: string; name: string; kind: string; marks: (string | null)[] }[];
};
...
async function main() {
  ...
  const links = [...html.matchAll(/<a[^>]+href="([^"]+\.pdf)"[^>]*>([\s\S]*?)<\/a>/gi)]
    ...
    // 「入所状況」（過去の実績）ではなく「空き状況一覧」を選ぶ
    .filter((l) => l.text.includes("空き状況一覧"));
  if (links.length !== 1) fail(`空き状況一覧のPDFが${links.length}件あります（1件のはず）`);
  const link = links[0];
  console.log(`PDF: ${link.text}\n  ${link.url}`);

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "higashihiroshima-vacancy-"));
  try {
    const r = await fetch(link.url, { headers: { "User-Agent": UA } });
    if (!r.ok) fail(`PDFの取得に失敗しました（${r.status}）: ${link.url}`);
    const buf = Buffer.from(await r.arrayBuffer());
    if (buf.subarray(0, 4).toString() !== "%PDF") fail(`PDFではありません: ${link.url}`);
    const file = path.join(tmpDir, "higashihiroshima.pdf");
    ...
    const symbolLegend = pdf.legend.map((l) => ({
      ...
      open: !squeeze(l.label).includes("空き無し"),
    }));
    ...
    const notes = [
      `東広島市は空き状況を人数ではなく記号で公表しています。これは${targetLabel}入所の受け入れ可能枠で、${asOf}時点のものです。`,
      ...pdf.notes,
