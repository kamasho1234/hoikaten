 ...
 * 桶川市の入所受入可能人数を取り込む
 *
 * 実行: npm run vacancy:fetch:okegawa
 *
 * ## この自治体の特徴
 * - 人数（受入可能人数）で公表
 * - 区分（公立・私立・認定こども園・小規模保育施設）で絞り込める
 * - 施設名が2行に折り返される（「カオルキッズランド保」＋「育園」）
 * - PDFのリンクの題が「令和8年9月入所受入可能人数」なので、そこから月を読む
 */
...
const MUNICIPALITY_SLUG = "okegawa";
const MUNICIPALITY_NAME = "桶川市";
const SOURCE_NAME = "桶川市「保育施設の空き状況」";
const INDEX_URL =
  "https://www.city.okegawa.lg.jp/soshiki/fukushi/hoiku/hoikusho/nyusho/5958.html";
const AGE_COUNT = 6;
const MIN_FACILITIES = 15;
const UA = "Mozilla/5.0 (compatible; hoikaten/1.0; +https://hoikaten.com)";

const OUT_PATH = path.join(process.cwd(), "src", "lib", "vacancy", `${MUNICIPALITY_SLUG}.json`);
const EXTRACTOR = path.join(process.cwd(), "scripts", "okegawa-pdf-extract.py");
...
async function main() {
  ...
  const links = [...html.matchAll(/<a[^>]+href="([^"]+\.pdf)"[^>]*>([\s\S]*?)<\/a>/gi)]
    ...
    .map((l) => {
      const m = /令和(\d+)年(\d+)月入所受入可能人数/.exec(l.text);
      return { ...l, key: m ? Number(m[1]) * 100 + Number(m[2]) : 0 };
    })
    .filter((l) => l.key > 0)
    .sort((a, b) => b.key - a.key);
  if (links.length === 0) fail("入所受入可能人数のPDFが見つかりません");
  const link = links[0];
  console.log(`PDF: ${link.text}
  ${link.url}`);

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "okegawa-vacancy-"));
  try {
    const r = await fetch(link.url, { headers: { "User-Agent": UA } });
    if (!r.ok) fail(`PDFの取得に失敗しました（${r.status}）: ${link.url}`);
    const buf = Buffer.from(await r.arrayBuffer());
    if (buf.subarray(0, 4).toString() !== "%PDF") fail(`PDFではありません: ${link.url}`);
    const file = path.join(tmpDir, "okegawa.pdf");
    ...
    const notes = [
      `桶川市が公開しているのは${targetLabel}入所の受入可能人数で、${asOf}時点のものです。`,
      ...pdf.notes,
    ];
