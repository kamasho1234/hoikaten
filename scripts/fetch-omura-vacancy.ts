 ...
 * 大村市の保育所等空き状況を取り込む
 *
 * 実行: npm run vacancy:fetch:omura
 *
 * ## この自治体の特徴
 * - 記号は ○＝空き3人以上、△＝空き1〜2人、×＝空きなし
 * - **PDFに「何日時点」が書かれていない**ので、公式ページの更新日を時点として使う。
 *   大村市は入所希望月の前々月に空き状況を出しているので、更新日＝公開日にあたる
 * - PDFのタイトルや注記に縦書き用の康熙部首（⼊ ⽉ ⽤）が混ざる。抽出側で直している
...
const MUNICIPALITY_SLUG = "omura";
const MUNICIPALITY_NAME = "大村市";
const SOURCE_NAME = "大村市「保育所などの空き状況一覧」";
const INDEX_URL =
  "https://www.city.omura.nagasaki.jp/kodomoriyo/kenko/kosodate/hoikujo/akijoukyou.html";
const AGE_COUNT = 6;
const MIN_FACILITIES = 50;
const UA = "Mozilla/5.0 (compatible; hoikaten/1.0; +https://hoikaten.com)";

const OUT_PATH = path.join(process.cwd(), "src", "lib", "vacancy", `${MUNICIPALITY_SLUG}.json`);
const EXTRACTOR = path.join(process.cwd(), "scripts", "omura-pdf-extract.py");
...
type PdfResult = {
  ...
  blanks: number;
  rows: { kind: string; name: string; marks: (string | null)[]; remark: string }[];
};
...
async function main() {
  ...
  const links = [...html.matchAll(/<a[^>]+href="([^"]+\.pdf)"[^>]*>([\s\S]*?)<\/a>/gi)]
    ...
    .filter((l) => l.text.includes("空き状況一覧表"));
  if (links.length !== 1) fail(`空き状況一覧表のPDFが${links.length}件あります（1件のはず）`);
  const link = links[0];
  console.log(`PDF: ${link.text}\n  ${link.url}`);

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "omura-vacancy-"));
  try {
    const r = await fetch(link.url, { headers: { "User-Agent": UA } });
    if (!r.ok) fail(`PDFの取得に失敗しました（${r.status}）: ${link.url}`);
    const buf = Buffer.from(await r.arrayBuffer());
    if (buf.subarray(0, 4).toString() !== "%PDF") fail(`PDFではありません: ${link.url}`);
    const file = path.join(tmpDir, "omura.pdf");
    ...
    for (const row of pdf.rows) {
      ...
      if (remark) remarks.push(`${name}: ${remark.replace(/^\*/, "")}`);
    ...
    const notes = [
      `大村市は空き状況を人数ではなく記号で公表しています。これは${targetLabel}入所を希望する人向けのもので、公式ページが${asOf}に更新されたものです。`,
      ...pdf.notes,
