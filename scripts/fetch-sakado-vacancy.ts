 ...
 * 坂戸市の保育所等の空き状況を取り込む
 *
 * 実行: npm run vacancy:fetch:sakado
 *
 * ## この自治体の特徴
 * - 記号は ○＝空きあり、×＝空きなし の2つだけ。**人数は非公表**
 * - 区分（公立保育園・私立保育園・小規模保育施設・認定こども園）は
 *   見出しの行として表の中に混ざる
 * - リンクの題が「保育所空き状況表（令和8年9月1日）」なので、そこから日付を読む
 */
...
const MUNICIPALITY_SLUG = "sakado";
const MUNICIPALITY_NAME = "坂戸市";
const SOURCE_NAME = "坂戸市「保育所等の空き状況」";
const INDEX_URL = "https://www.city.sakado.lg.jp/site/sakado-kosodate/41080.html";
const AGE_COUNT = 6;
const MIN_FACILITIES = 20;
const UA = "Mozilla/5.0 (compatible; hoikaten/1.0; +https://hoikaten.com)";

const OUT_PATH = path.join(process.cwd(), "src", "lib", "vacancy", `${MUNICIPALITY_SLUG}.json`);
const EXTRACTOR = path.join(process.cwd(), "scripts", "sakado-pdf-extract.py");
...
async function main() {
  ...
  const links = [...html.matchAll(/<a[^>]+href="([^"]+\.pdf)"[^>]*>([\s\S]*?)<\/a>/gi)]
    ...
    .map((l) => {
      const m = /空き状況表[（(]令和(\d+)年(\d+)月(\d+)日[）)]/.exec(l.text);
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "sakado-vacancy-"));
  try {
    const r = await fetch(link.url, { headers: { "User-Agent": UA } });
    if (!r.ok) fail(`PDFの取得に失敗しました（${r.status}）: ${link.url}`);
    const buf = Buffer.from(await r.arrayBuffer());
    if (buf.subarray(0, 4).toString() !== "%PDF") fail(`PDFではありません: ${link.url}`);
    const file = path.join(tmpDir, "sakado.pdf");
    ...
    // リンクの題の日付とPDFの中の入所日が合っているか
    const [reiwa, month, day] = pdf.target;
    if (link.key !== reiwa * 10000 + month * 100 + day) {
      fail(
        `リンクの題（${link.text}）とPDFの中の入所日（令和${reiwa}年${month}月${day}日）が違います`
      );
    }
    ...
    const notes = [
      `坂戸市は空き状況を人数ではなく「空きあり」「空きなし」の記号で公表しています。これは${targetLabel}入所のもので、公式ページが${asOf}に更新されたものです。`,
      ...pdf.notes,
