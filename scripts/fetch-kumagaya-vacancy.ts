/**
 * 熊谷市の保育所等の受入可能状況を取り込む
 *
 * 実行: npm run vacancy:fetch:kumagaya
 *
 * ## この自治体の特徴
 * - 記号（○＝2人以上、△＝1人、×＝空きはありません）
 * - 「年齢」の列（「8週～」「6か月～2歳」など）があるので、
 *   受け入れていない歳児と切り分けられる
 * - 区分（公立保育所・民間保育園・認定こども園・地域型保育施設）が縦結合
 */
...
const MUNICIPALITY_SLUG = "kumagaya";
const MUNICIPALITY_NAME = "熊谷市";
const SOURCE_NAME = "熊谷市「認定こども園・保育所(園)・地域型保育施設 受入可能状況」";
const INDEX_URL =
...
const COL_NO = 1;
const COL_NAME = 2;
const COL_TARGET_AGE = 5;
const COL_AGE0 = 7;

const OUT_PATH = path.join(process.cwd(), "src", "lib", "vacancy", `${MUNICIPALITY_SLUG}.json`);
const EXTRACTOR = path.join(process.cwd(), "scripts", "kumagaya-pdf-extract.py");
...
/** 「8週～」「6か月～2歳」「満2歳～」から [下限, 上限] を取る */
function targetAges(raw: string): [number, number] | null {
  const t = toHalfWidth(squeeze(raw)).replace(/[～〜~]/g, "~");
  const parts = t.split("~");
  if (parts.length === 0 || parts[0] === "") return null;
  const low = /^\d+(週|か月|ヶ月|ヵ月|日)$/.test(parts[0])
    ? 0
    : Number(parts[0].match(/^満?(\d+)歳$/)?.[1] ?? NaN);
  if (!Number.isInteger(low)) return null;
  if (parts.length === 1 || parts[1] === "") return [low, AGE_COUNT - 1];
  const high = Number(parts[1].match(/^(\d+)歳$/)?.[1] ?? NaN);
  if (!Number.isInteger(high) || high < low || high >= AGE_COUNT) return null;
  return [low, high];
}
...
async function main() {
  ...
  const links = [...html.matchAll(/<a[^>]+href="([^"]+\.pdf)"[^>]*>([\s\S]*?)<\/a>/gi)]
    ...
    .map((l) => {
      const m = squeeze(l.text).match(/令和(\d+)年(\d+)月入所受入可能状況/);
      if (!m) return null;
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "kumagaya-vacancy-"));
  try {
    const r = await fetch(latest.url, { headers: { "User-Agent": UA } });
    if (!r.ok) fail(`PDFの取得に失敗しました（${r.status}）: ${latest.url}`);
    const buf = Buffer.from(await r.arrayBuffer());
    if (buf.subarray(0, 4).toString() !== "%PDF") fail(`PDFではありません: ${latest.url}`);
    const file = path.join(tmpDir, "kumagaya.pdf");
    ...
    for (const row of pdf.rows) {
      ...
      // 「年齢」の欄は「8週～」のように上限を書かないことが多く、
      // 実際のクラス構成は園ごとに違う（4・5歳のない園もある）。
      // 欄が空かどうかでクラスの有無を判断し、年齢は読めることだけ確かめる
      if (!targetAges(row[COL_TARGET_AGE])) {
        fail(`${name}: 年齢を読めません: 「${row[COL_TARGET_AGE]}」`);
      }
    ...
    console.log(`通し番号は1〜${sorted.length}の連番、記号の数はPDFの文字と一致しました`);
    ...
    const dataset = {
      ...
      notes: [
        "熊谷市は空き状況を人数ではなく記号で公表しています。当サイトでも公式の記号のまま載せています。",
        "目安であり、施設ごとの職員の配置状況や転退所の状況により変わることがあります。",
        "認定こども園の教育・保育給付1号認定の入所や、在園児の認定変更については各施設にご確認ください。",
        "年齢はその年度の4月1日時点のものです。受け入れ対象の年齢の外は「—」にしています。",
      ],
      wards: [],
      categories,
      symbolLegend,
    ...
    console.log(`  受け入れ対象の年齢の外: ${noClass}`);
    console.log("");
