 ...
 * 印西市の保育園空き状況を取り込む
 *
 * 実行: npm run vacancy:fetch:inzai
 *
 * ## この自治体の特徴
 * - **記号（空き状況）と人数（在園児数）の両方**が載っている。
 *   空きは記号なので symbol、在園児数は enrolled として持つ
 * - 記号は ○＝4人以上空きあり、△＝1〜3人の空きあり、×＝空きなし
 * - 区分（公立認可保育園・私立認可保育園・認定こども園保育部分・小規模保育園）で
 *   絞り込める
 * - 月ごとのPDFがページに積み上がるので、ファイル名の「R8.10」がいちばん新しいものを選ぶ
 */
...
const MUNICIPALITY_SLUG = "inzai";
const MUNICIPALITY_NAME = "印西市";
const SOURCE_NAME = "印西市「市内認可保育園等（公立・私立）の空き状況」";
const INDEX_URL = "https://www.city.inzai.lg.jp/kosodatenavi/0000019458.html";
const AGE_COUNT = 6;
const MIN_FACILITIES = 35;
const UA = "Mozilla/5.0 (compatible; hoikaten/1.0; +https://hoikaten.com)";

const OUT_PATH = path.join(process.cwd(), "src", "lib", "vacancy", `${MUNICIPALITY_SLUG}.json`);
const EXTRACTOR = path.join(process.cwd(), "scripts", "inzai-pdf-extract.py");
...
async function main() {
  ...
  // ファイル名が「R8.10akizyoukyou.pdf」のように年月になっている
  const links = [...html.matchAll(/href="([^"]*R(\d+)\.(\d+)[^"/]*akizyoukyou\.pdf)"/gi)].map((m) => ({
    url: new URL(m[1], res.url || INDEX_URL).toString(),
    key: Number(m[2]) * 100 + Number(m[3]),
  }));
  if (links.length === 0) fail("空き状況のPDF（R年.月akizyoukyou.pdf）が見つかりません");
  links.sort((a, b) => b.key - a.key);
  const link = links[0];
  console.log(`PDF: ${link.url}`);

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "inzai-vacancy-"));
  try {
    const r = await fetch(link.url, { headers: { "User-Agent": UA } });
    if (!r.ok) fail(`PDFの取得に失敗しました（${r.status}）: ${link.url}`);
    const buf = Buffer.from(await r.arrayBuffer());
    if (buf.subarray(0, 4).toString() !== "%PDF") fail(`PDFではありません: ${link.url}`);
    const file = path.join(tmpDir, "inzai.pdf");
    ...
    // ファイル名の年月とPDFの中の入園月が合っているか
    if (link.key !== pdf.target[0] * 100 + pdf.target[1]) {
      fail(
        `ファイル名（R${Math.floor(link.key / 100)}.${link.key % 100}）とPDFの中の入園月（令和${pdf.target[0]}年${pdf.target[1]}月）が違います`
      );
    }
    ...
    // 検算2: 在園児数の合計がPDFの「合 計」の行と合うか
    for (let age = 0; age < AGE_COUNT; age++) {
      const sum = facilities.reduce((acc, f) => acc + (f.enrolled[age] ?? 0), 0);
      if (sum !== (pdf.totals[age] ?? 0)) {
        fail(`在園児数の${age}歳児が合計と合いません（合計 ${pdf.totals[age]} / 取り込み ${sum}）`);
      }
    }
    ...
    console.log("記号の数と在園児数の合計が、どちらもPDFと一致しました");
    ...
    const notes = [
      `印西市は空きを人数ではなく記号で公表しています。これは${targetLabel}入園に向けたもので、${asOf}時点のものです。あわせて在園児数も載せています。`,
    ...
    const dataset = {
      ...
      subtitle: `${targetLabel}入園に向けた空き状況`,
      notes,
      wards: [],
      categories,
      symbolLegend,
