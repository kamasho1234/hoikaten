 ...
 * 鳥栖市の保育所等の空き状況を取り込む
 *
 * 実行: npm run vacancy:fetch:tosu
 *
 * ## この自治体の特徴
 * - 記号は ○＝3人以上空きあり、△＝1〜2人空きあり、×＝空きなし
 * - **区分の見出しの行に凡例も一緒に書かれている**
 *   （「（１）認可保育所 ○：3人以上空きあり、…」）
 * - 小学校区で絞り込める。「若葉・田代※１」のように注の番号が付くことがある
 * - 区分名に「地域型保育事業（小規模A…小A、…）」と略号の説明が付くので、
 *   括弧の中は落として注記に回す
 */
...
const MUNICIPALITY_SLUG = "tosu";
const MUNICIPALITY_NAME = "鳥栖市";
const SOURCE_NAME = "鳥栖市「保育所等の空き情報」";
const INDEX_URL = "https://www.city.tosu.lg.jp/soshiki/9/1055.html";
const AGE_COUNT = 6;
const MIN_FACILITIES = 20;
const UA = "Mozilla/5.0 (compatible; hoikaten/1.0; +https://hoikaten.com)";

const OUT_PATH = path.join(process.cwd(), "src", "lib", "vacancy", `${MUNICIPALITY_SLUG}.json`);
const EXTRACTOR = path.join(process.cwd(), "scripts", "tosu-pdf-extract.py");
...
type PdfResult = {
  ...
  groups: {
    kind: string;
    rows: { name: string; town: string; school: string; marks: (string | null)[] }[];
  }[];
};
...
async function main() {
  ...
  const links = [...html.matchAll(/<a[^>]+href="([^"]+\.pdf)"[^>]*>([\s\S]*?)<\/a>/gi)]
    ...
    .map((l) => {
      const m = /空き状況[（(]令和(\d+)年(\d+)月(\d+)日更新/.exec(l.text);
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "tosu-vacancy-"));
  try {
    const r = await fetch(link.url, { headers: { "User-Agent": UA } });
    if (!r.ok) fail(`PDFの取得に失敗しました（${r.status}）: ${link.url}`);
    const buf = Buffer.from(await r.arrayBuffer());
    if (buf.subarray(0, 4).toString() !== "%PDF") fail(`PDFではありません: ${link.url}`);
    const file = path.join(tmpDir, "tosu.pdf");
    ...
    const symbolLegend = pdf.legend.map((l) => ({
      ...
      open: !squeeze(l.label).startsWith("空きなし"),
    }));
    ...
    for (const group of pdf.groups) {
      // 「地域型保育事業（小規模A…小A、…）」の括弧の中は略号の説明なので注記に回す
      const full = squeeze(group.kind);
      const kind = full.replace(/[（(].*$/, "");
      if (!kind) fail(`区分の名前が空です（「${group.kind}」）`);
      if (full !== kind) kindNotes.push(full);
    ...
    const notes = [
      `鳥栖市は空き状況を人数ではなく記号で公表しています。これは${targetLabel}入所分で、${asOf}時点のものです。`,
      ...pdf.notes,
      ...kindNotes,
      "小学校区は、園が所在する町区の属する小学校区です。",
