/**
 * 行田市の保育所等の受入枠の状況を取り込む
 *
 * 実行: npm run vacancy:fetch:gyoda
 *
 * ## この自治体の特徴
 * - 記号は ○＝あり、×＝なし の2つだけ（人数は出さない）
 * - 区分（保育所・認定こども園・地域型保育事業所）で絞り込める
 * - PDFのファイル名が「202609ukeire.pdf」のように年月になっているので、
 *   いちばん新しいものを選ぶ
 */
...
const MUNICIPALITY_SLUG = "gyoda";
const MUNICIPALITY_NAME = "行田市";
const SOURCE_NAME = "行田市「保育所等の受入可能状況について」";
const INDEX_URL =
  "https://www.city.gyoda.lg.jp/soshiki/kenkouhukushibu/kodomomirai/gyomu/ikuzi_misyugaku/hoikusho_kodomoen/hoikusho/7906.html";
const AGE_COUNT = 6;
const MIN_FACILITIES = 15;
const UA = "Mozilla/5.0 (compatible; hoikaten/1.0; +https://hoikaten.com)";

const OUT_PATH = path.join(process.cwd(), "src", "lib", "vacancy", `${MUNICIPALITY_SLUG}.json`);
const EXTRACTOR = path.join(process.cwd(), "scripts", "gyoda-pdf-extract.py");
...
async function main() {
  ...
  // ファイル名が「202609ukeire.pdf」のように年月になっている
  const links = [...html.matchAll(/href="([^"]*?(\d{4})(\d{2})ukeire\.pdf)"/gi)].map((m) => ({
    url: new URL(m[1], res.url || INDEX_URL).toString(),
    key: Number(m[2]) * 100 + Number(m[3]),
  }));
  if (links.length === 0) fail("受入可能状況のPDF（YYYYMMukeire.pdf）が見つかりません");
  links.sort((a, b) => b.key - a.key);
  const link = links[0];
  console.log(`PDF: ${link.url}`);

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "gyoda-vacancy-"));
  try {
    const r = await fetch(link.url, { headers: { "User-Agent": UA } });
    if (!r.ok) fail(`PDFの取得に失敗しました（${r.status}）: ${link.url}`);
    const buf = Buffer.from(await r.arrayBuffer());
    if (buf.subarray(0, 4).toString() !== "%PDF") fail(`PDFではありません: ${link.url}`);
    const file = path.join(tmpDir, "gyoda.pdf");
    ...
    // ファイル名の年月とPDFの中の入所月が合っているか
    const targetYear = reiwaToYear(pdf.target[0]);
    if (link.key !== targetYear * 100 + pdf.target[1]) {
      fail(
        `ファイル名（${Math.floor(link.key / 100)}年${link.key % 100}月）とPDFの中の入所月（${targetYear}年${pdf.target[1]}月）が違います`
      );
    }
    const targetLabel = `${targetYear}年${pdf.target[1]}月`;
    console.log(`時点: ${asOf} ／ 対象: ${targetLabel}入所`);

    const symbolLegend = pdf.legend.map((l) => ({
      mark: shapeOf(squeeze(l.mark)),
      label: squeeze(l.label),
      open: squeeze(l.label) !== "なし",
    }));
    if (symbolLegend.length !== 2) fail(`凡例が${symbolLegend.length}件です（2件のはず）`);
    if (!symbolLegend.some((l) => l.open)) fail("受入枠ありの記号が凡例にありません");
    ...
    const notes = [
      `行田市は空き状況を人数ではなく「あり」「なし」の記号で公表しています。これは${targetLabel}入所の受入可能状況の見込みで、${asOf}時点のものです。`,
      ...pdf.notes,
    ...
    const dataset = {
      ...
      metrics: ["symbol"],
      subtitle: `${targetLabel}入所の受入枠の状況`,
      notes,
      wards: [],
      categories,
      symbolLegend,
