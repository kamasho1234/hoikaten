...
## 施設サイトのリンク状況（2026-08-17に全1,001件へ実アクセスして確認）

- **1,242施設中1,001施設（80.6%）にリンクあり**。type別は 運営法人516・園そのもの450・横浜市のページ35。ユニークドメイン446。
- **未カバー241施設は「未調査」ではなく「調査したが確定できなかった」もの**。
  全1,573レコードの調査結果は verified 1,029・no_url 327（サイトが存在しない）・unverified 160（名前も電話も一致しない）・
  unreachable 56・denied_domain 1。小規模保育室・認定こども園・幼稚園・市立園に多い。
- 区別カバー率は 青葉区57.6%・戸塚区63.8% が低く、鶴見区/西区92.7%・保土ケ谷区91.4% が高い。
- **生存確認: 978/1,001が到達OK（うち136件はリダイレクト）。404は0件。** 要確認は23件のみ:
  - **`care-partner.com`（キッズパートナー）19件**: 中間証明書が不完全で Node が `UNABLE_TO_VERIFY_LEAF_SIGNATURE`。
    ただし同じドメインでも通常アクセスで200が返るものがあり、TLS検証を切れば全て200・タイトルも園名と一致。
    Chrome は中間証明書を補完するため実害は小さいが、ブラウザによっては警告が出る。
  - **`kirara-hoikuen.com`（雲母保育園）3件**: **サイト全体が500**（トップも `/about/hoikuen/` も500・本文0バイト）。
    URLの誤りではなくサーバー障害の可能性が高いため据え置く。**後日再確認して復旧しなければリンクを外す。**
  - めばえ横浜保育園1件: 一度タイムアウトしたが再試行で200・タイトル一致。問題なし。
- 別ドメインへの転送17件はいずれも同一法人内の正当な移動（`nihonhoiku.co.jp`→`list.nihonhoiku.co.jp` など）。
  ただし**京進のほいくえんＨＯＰＰＡ反町園だけは `hoppa.jp/tanmachi/` が法人トップ（keceg.jp）に着地する**（園別ページが消えた）。
  200は返るので残しているが、実態は `type: "corp"` に近い。
